...
function generateOfflineAiResponse(userMessage: string, catalog: any[]): { reply: string; recommendedIds: string[] } {
  ...
  const shxrk = findProd("shxrk") || catalog.find((p) => p.id === "setting-shxrkxinside");
  const lowLatency = findProd("latency") || catalog.find((p) => p.id === "pokky-low-latency");
  const network = findProd("network") || catalog.find((p) => p.id === "stepha-network-setting");
  const stephaOS = findProd("stepha") || catalog.find((p) => p.id === "setting-stepha");
  const projectS = findProd("project") || catalog.find((p) => p.id === "project-s-v7");
  const timerRes = findProd("timer") || catalog.find((p) => p.id === "timer-resolution");
...
1. Communicate politely, enthusiastically, and professionally in Thai language (ครับ/ผม). Use clear formatting with bullet points and emojis.
...
Use ONLY the exact IDs listed in the catalog above. If no specific product is relevant, you can omit the tag or recommend the most suitable one (like "pokky-low-latency").`;
